...
var Zepto = (function () {
    ...
    zepto.matches = function (element, selector) {
        ...
        var match, parent = element.parentNode,
            temp = !parent
        if (temp)(parent = tempParent)
            .appendChild(element)
        match = ~zepto.qsa(parent, selector)
            .indexOf(element)
    ...
    function defaultDisplay(nodeName) {
        ...
        if (!elementDisplay[nodeName]) {
            ...
            display = getComputedStyle(element, '')
    ...
    zepto.init = function (selector, context) {
        ...
        // Optimize for string selectors
            ...
            // nodes from there
            else if (context !== undefined) return $(context)
                .find(selector)
            // If it's a CSS selector, use it to select nodes.
            else dom = zepto.qsa(document, selector)
        }
        // If a function is given, call it when the DOM is ready
        else if (isFunction(selector)) return $(document)
            .ready(selector)
    ...
    function filtered(nodes, selector) {
        return selector == null ? $(nodes) : $(nodes)
            .filter(selector)
    }
    ...
    $.fn = {
        ...
        show: function () {
            return this.each(function () {
                ...
                if (getComputedStyle(this, '')
                    .getPropertyValue("display") == "none")
                    this.style.display = defaultDisplay(this.nodeName)
            })
        },
        replaceWith: function (newContent) {
            return this.before(newContent)
                .remove()
        },
        ...
        wrapAll: function (structure) {
            if (this[0]) {
                $(this[0])
                    .before(structure = $(structure))
                var children
                // drill down to the inmost element
                while ((children = structure.children())
                    .length) structure = children.first()
                $(structure)
                    .append(this)
            }
            return this
        ...
        offset: function (coordinates) {
            if (coordinates) return this.each(function (index) {
                var $this = $(this),
                    ...
                    parentOffset = $this.offsetParent()
                    .offset(),
                    props = {
            ...
            return this.each(function (idx) {
                ...
                newName.split(/\s+/g)
                    .forEach(function (klass) {
                        if (!$(this)
                            .hasClass(klass)) classList.push(klass)
                    }, this)
        ...
        position: function () {
            ...
            offset.top -= parseFloat($(elem)
                .css('margin-top')) || 0
            offset.left -= parseFloat($(elem)
                .css('margin-left')) || 0
        ...
        offsetParent: function () {
            return this.map(function () {
                ...
                while (parent && !rootNodeRE.test(parent.nodeName) && $(parent)
                    .css("position") == "static")
...
(function ($) {
    ...
    function add(element, events, fn, data, selector, delegator, capture) {
        ...
        events.split(/\s/)
            .forEach(function (event) {
                if (event == 'ready') return $(document)
                    .ready(fn)
    ...
    $.fn.live = function (event, callback) {
        $(document.body)
            .delegate(this.selector, event, callback)
        return this
    }
    $.fn.die = function (event, callback) {
        $(document.body)
            .undelegate(this.selector, event, callback)
        return this
    }
    ...
    $.fn.anim = function (properties, duration, ease, callback, delay) {
        ...
        wrappedCallback = function (event) {
            if (typeof event !== 'undefined') {
                ...
                $(event.target)
                    .unbind(endEvent, wrappedCallback)
            } else
                $(this)
                .unbind(endEvent, wrappedCallback) // triggered by setTimeout
